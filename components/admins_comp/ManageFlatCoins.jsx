...
const ManageFlatCoins = ({ assets, refetch }) => {
  ...
  const handleCoinChange = (e) => {
    ...
    if (type === "flat coin") {
      ...
      secureAPI
        .put(`/allFlatCoins/${assetId}`, coinInfo)
        .then((res) => {
      ...
      // console.log(coinInfo)
      secureAPI
        .put(`/allCryptoCoins/${assetId}`, coinInfo)
        .then((res) => {
          if (res.data.modifiedCount) {
            Swal.fire({
              title: `Edit successful!`,
  ...
  return (
    <div className="gap-6 grid md:grid-cols-2 2xl:grid-cols-3  3xl:grid-cols-4 my-6">
        ...
        >
          ...
          <label htmlFor="icon">
            <p className="border-2 p-2 rounded w-max cursor-pointer">
              {" "}
              <Image width={50} height={50} src={defaultIcon} alt="coin-icon" />
              Change Icon
            </p>
          </label>
        </DialogContent>
      ...
      {assets.map((asset, idx) => (
        ...
        <div
          key={idx}
          className="coinBg rounded-lg p-6 space-y-4 min-w-[280px] relative bg-sky-200/30 dark:bg-darkOne dark:border  dark:border-darkThree dark:text-white shadow"
        >
          ...
          <p className="font-semibold">
            Current value: {parseFloat(asset.price).toFixed(2)}{" "}
            <span className="text-xs text-indigo-400">{asset.key}/USD</span>
          </p>
        </div>
      ))}
    </div>
